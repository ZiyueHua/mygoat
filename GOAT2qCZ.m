...
u=cat( 1, 0.01/n_har.*(rand(n_har*length(Hs),1)-0.5), pi/(tstop-tstart)/10.*ones(length(Hs),1), zeros(length(Hs),1) );
save( filename, 'u');
% load(filename, 'u');
u0=u;

penaltyl=0;
h=cat( 1, 0.01/n_har*ones(n_har*length(Hs),1), pi/dt/n_har*ones(length(Hs),1) );
...
options = optimoptions(options,'MaxFunctionEvaluations', 1e900);
options = optimoptions(options,'DerivativeCheck', 'off');
options = optimoptions(options,'MaxIterations', 1e900);
options = optimoptions(options,'OptimalityTolerance', 1e-900);
options = optimoptions(options,'FunctionTolerance', 1e-900);
options = optimoptions(options,'StepTolerance', 1e-900);
